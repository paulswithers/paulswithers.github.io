---
layout: post
title: "Paul Withers Blog: Thoughts on Domino"
date: 2016-07-13
---
The key to any relationship is periodically stepping back and appreciating the good points in contrast to the little annoyances that grate, so that you're not distracted by the first pretty young face (or muscular torso, depending on your predilection) that you encounter. When you're thinking it's time to leave the relationship is the most crucial time to evaluate honestly and dispassionately what you have / had. Because if you don't, sooner or later you'll find different annoyances that grate; you'll find something you took for granted and absolutely needed is missing from your new love; and before you know it that love too will turn sour and you'll be crying into your alcohol bemoaning wasted years and shattered dreams while looking at a bank account that's been wiped out by periodic divorce settlements.

So I'm taking a few moments to evaluate what Domino provides, what I use to a greater or lesser extent, particularly what I take for granted, to better identify what's important. Maybe what I provide here will also be of benefit to other developers or customers.
...
But the people who said big data meant RDBMS have since moved to graph. OpenNTF Domino API has had graph in development for over a year, I've used it on a number of occasions, and the ability to combine NoSQL data and graph data, using Java interfaces for easy schema management and out-of-the-box componentisation of data objects, gives a huge degree of flexibility. And all without requiring any view indices.

### Domino API (Java / LotusScript / C)

The NoSQL database also includes data items that manage both architecture as well as data. In terms of "design", this means Domino's NoSQL design includes CRUD events, business logic, workflow routing and database management. This means not just basic REST services like those exposed for mail and calendaring, but a complete API to do more than update values passed from the UI to the database layer.
...
If you're developing using a JavaScript framework and all you want is very basic CRUD, DAS is available to deliver access to documents and views via simple configuration. Needless to say from what I covered above and for anyone who's read my blogging for Intec, DAS is not for me.
...
Databases can also be clustered within a domain.
...
Most Domino developers use only the full stack. For some time, some have been using Domino as just a database and web server. Others have been using Domino as middleware, using its runtime but a different backend database. More recently, I've been experimenting with Websphere Liberty as the web server, Vaadin as the development framework and Domino just as a database storing data as graph plus server for SMTP / scheduling / auditing etc. Removing Domino for me, someone not extremely familiar with alternatives, leaves a lot of learning, questions, pitfalls, concerns and (I suspect) painful learning. For anyone who uses the whole stack and has not pushed their learning over recent years, it must be terrifying.
...
Websphere Liberty is a modern lightweight server (I have several running on my PC) that allows modern Java EE development with Java 8 and can deploy web applications as standard web archives. The last one is something I've missed when moving back to Domino. The server can be configured via XML files or an Eclipse plugin which provides editors. I've not gone beyond basic configuration so far, so I know a real-world deployment would require a lot of questions answering, possible pitfalls and potential optimisation that's required. But initial setup is very easy, easy enough to let me focus on doing the development I want. With the CrossWorlds approach, it still gives access to Domino while getting a modern HTTP server without needing an additional physical or virtual server, which would minimise expense of replacing part of the Domino stack. It's a nice environment, but may not appeal to most customers. Apache Tomcat is an obvious other server to consider, when there is time and need.

In terms of graph, I like the conciseness of the framed graph data schema from Tinkerpop 2 (dropped for Tinkerpop 3, but there are open source framing implementations), and I like the native Java graph API in OpenNTF Domino API. I've been looking at alternative graph APIs that are also based on Tinkerpop. I wasn't impressed by IBM Graph, struggled with OrientDb, so I'm looking now at Apache Cassandra.
