...
Remember that we added a shadow DOM and we need to query its DOM through that, through `this.root` which was set in the constructor. Then we create a div for each tile and add an event listener. We saw in [part three]({{ site.baseurl }}{% link _posts/2024-08-24-framework-web-3.md %}) that event listeners are the modern approach. For ease, we call a function that we store in a property of the class.
