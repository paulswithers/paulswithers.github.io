...
But once your site is ready for publication, you can just use `mkdocs build`. If you're using a Docker container, you can just pass `build` instead of `serve` when creating the temporary container. The result will be a `site` directory containing your website. This can then just be copied and pasted into your **WebContent\WEB-INF\site** directory, as detailed in ["GitHub Pages Sites on Domino 3: How"](/blog/2022/08/17/github-pages-on-domino-3).
