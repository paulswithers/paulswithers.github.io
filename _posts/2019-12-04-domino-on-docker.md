...
About a year ago I did a blog post on Domino on Docker, with the intention to follow it up on developing against that Domino Docker server via Notes Client and integrating with it from other Docker containers or outside of Docker. Unfortunately other things got in the way, and it then got put on hold pending the work Thomas Hampel and Daniel Nashed were doing on the [IBM repository](https://github.com/IBM/domino-docker) for Domino on Docker and Roberto Boccadero's Domino on Docker guide posted on [OpenNTF's wiki guides site](https://wiki.openntf.org/display/OPH/Community+Guides).
